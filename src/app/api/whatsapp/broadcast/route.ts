...
export async function POST(request: Request) {
  try {
    ...
    // Super Safe holds this account's outbound broadcast messages to
    // ~8/min via the shared token bucket (per-recipient, server-side) —
    // "instant" posts each batch straight to Meta, bounded only by
    // provider limits. Defaults to instant so legacy callers and batches
    // sent without the field behave exactly as before.
    const sendingMode: BroadcastSendingMode = isBroadcastSendingMode(mode)
      ? mode
      : DEFAULT_BROADCAST_SENDING_MODE
    ...
    for (const recipient of recipients) {
      // Super Safe mode: wait for this account's ~8/min token BEFORE
      // touching Meta, so the pace is decided and enforced here on the
      // server — not by a browser timer.
      if (sendingMode === 'super_safe') {
        await awaitSuperSafeToken(accountId)
      }
