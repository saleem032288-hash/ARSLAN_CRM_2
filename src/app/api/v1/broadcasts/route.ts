...
//     "mode": "instant",                    // optional: "instant" | "super_safe"
    ...
    const plan = await createBroadcast(ctx.supabase, ctx.accountId, auditUserId, {
      ...
      templateLanguage:
        typeof body.template_language === 'string'
          ...
          : null,
      // "instant" (default) ships as fast as Meta allows; "super_safe"
      // paces every message at ~8/min through the server-side token
      // bucket, regardless of who is watching.
      mode: body.mode === 'super_safe' ? 'super_safe' : 'instant',
