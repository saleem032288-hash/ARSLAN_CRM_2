...
export async function planBroadcastResume(
  ...
  const plan: BroadcastPlan = {
    ...
    rejected: 0,
    // The wizard writes the mode onto `audience_filter`; a resume of
    // a Super Safe campaign keeps pacing (~8/min) instead of blasting
    // the leftovers out at full speed.
    mode: extractSendingMode(broadcast.audience_filter),
  };
