...
interface BroadcastPayload {
  ...
  /**
   * Sending mode. `super_safe` paces at ~8 messages/minute and is
   * enforced by the SERVER (the token bucket in
   * `lib/whatsapp/super-safe-rate-limit.ts`) — the batch size and
   * inter-batch delay below only change to match that pace, they don't
   * set it.
   */
...
/**
 * Super Safe mode posts one recipient per request and relies on the
 * server-side token bucket (~8/min, `lib/whatsapp/super-safe-rate-limit.ts`)
 * for the actual pacing — no inter-batch sleep is needed because each
 * round-trip's token wait is the delay. 1-recipient batches also keep
 * a single request short instead of holding one HTTP connection open
 * for 10 × ~7.5s of Meta sends.
 */
const SUPER_SAFE_BATCH_SIZE = 1;
const SUPER_SAFE_BATCH_DELAY_MS = 0;
...
export function useBroadcastSending(): UseBroadcastSendingReturn {
  ...
  async function createAndSendBroadcast(payload: BroadcastPayload): Promise<string> {
    ...
    try {
      ...
      const { data: broadcast, error: broadcastError } = await supabase
        ...
        .insert({
          ...
          audience_filter: {
            ...
            // Surfaced on the row so a server-side resume of an
            // abandoned campaign keeps pacing in the chosen mode
            // (reused jsonb column — no schema change).
            mode: payload.mode,
          },
          status: 'sending',
      ...
      // Batch shape follows the mode. The server enforces Super Safe
      // (~8/min) per recipient; Instant uses the historical 10-per-batch
      // cadence (issue #472) that keeps us within Meta's per-number
      // limits. The mode also ships on every request body.
      const batchSize =
        payload.mode === 'super_safe' ? SUPER_SAFE_BATCH_SIZE : SEND_BATCH_SIZE;
      const batchDelay =
        payload.mode === 'super_safe'
          ? SUPER_SAFE_BATCH_DELAY_MS
          : SEND_BATCH_DELAY_MS;
